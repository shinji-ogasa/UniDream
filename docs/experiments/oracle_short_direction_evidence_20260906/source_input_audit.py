...
if __name__=='__main__':
 import os,sys
 root=Path('/Users/sophie/Documents/UniDream/.worktrees/oracle-feature-frontier-20260905');os.chdir(root)
 cfg={'parent_config':'configs/oracle_soft_direction_decisions_20260906.yaml','feature_config':'configs/oracle_short_feature_decisions_20260906.yaml',
  'source_bindings':{p:sha(p) for p in sorted(set(soft.SOURCES)|set(short.SOURCES))},
  'parent_manifest_bindings':{p:sha(p) for p in sorted(manifest_paths(ROOT19))},
  'feature_manifest_bindings':{p:sha(p) for p in sorted(manifest_paths(ROOT15))},'preflight_sha256':None}
 for kind in ('parent','feature'):cfg[kind+'_config_sha256']=sha(cfg[kind+'_config'])
 assert cfg['parent_manifest_bindings'][str(ROOT19/'results.json')]=='d85e14e1a8249601a1f28c0d4fa29b1fbb23a3571232dcedc5edee0644b82cc5'
 assert cfg['feature_manifest_bindings'][str(ROOT15/'results.json')]=='2f65382455e1266e035d38007a8dc939efb74d84df676c44da8ed3de9cd77218'
 result=prepare_sources(cfg);pre=result['preflight']
 output={'passed':True,'script_sha256':sha(__file__),'audit_config':cfg,**pre}
 path=Path('/tmp/oracle_short_direction_source_audit_20260906.json');path.write_text(json.dumps(output,sort_keys=True,indent=2,allow_nan=False)+'\n')
 print(json.dumps({'path':str(path),'sha256':sha(path),'script_sha256':sha(__file__),'inventory':pre['inventory'],'no_new_fits_statistics_predictions_losses_or_orders':True}),flush=True)
