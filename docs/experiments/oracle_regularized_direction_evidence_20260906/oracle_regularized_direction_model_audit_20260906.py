"""Independent Stage18 fixed-normalized-L2 model audit; default synthetic, real mode only on root GO.
...
OUT=Path('codex_outputs/oracle_regularized_direction_decisions_v1')
PARENT=Path('codex_outputs/oracle_direction_decisions_v1')
CONFIG=Path('configs/oracle_regularized_direction_decisions_20260906.yaml')
PARITY=Path('codex_outputs/oracle_frozen_procedure_parity_v1')
REPORT=Path('/tmp/oracle_regularized_direction_model_audit_20260906.json')
...
def objective_gradient_hessian(x,labels,weights,beta,intercept,c):
    """x is independently transformed; no BLAS for objective/derivatives."""
    total=float(np.sum(weights));p=len(beta);l2=1.0/(float(c)*total)
    assert math.isfinite(l2) and l2>0
    ...
    objective=math.fsum(loss)+.5*l2*math.fsum(float(b)**2 for b in beta)
    gradient=[math.fsum(r*row[j] for r,row in zip(residual,x))+l2*float(beta[j]) for j in range(p)]
    ...
    hessian=np.asarray([[math.fsum(w*row[j]*row[k] for w,row in zip(curvature,design))
                         +(l2 if j==k and j<p else 0.) for k in range(p+1)] for j in range(p+1)])
...
def synthetic():
    ...
    objective,g,h=objective_gradient_hessian(x,labels,w,[0.],math.log(3.),.25)
    assert max(abs(g))<1e-15 and math.isfinite(objective) and np.linalg.eigvalsh(h).min()>0
    _,g,_=objective_gradient_hessian(x,labels,np.asarray([3.,1.,1.,1.]),[0.],0.,1/6)
    ...
    # A nonzero coefficient distinguishes the general penalty from old beta/W.
    x=[[-1.],[.5],[1.],[2.]];w=np.asarray([.5,1.,1.,1.5]);labels=[0,0,1,1]
    o,g,h=objective_gradient_hessian(x,labels,w,[.3],-.2,1/float(np.sum(w)))
    oo,gg,hh=objective_gradient_hessian(np.repeat(x,2,axis=0),np.repeat(labels,2),np.repeat(w,2),[.3],-.2,1/float(np.sum(np.repeat(w,2))))
    assert abs(o-oo)<1e-15 and np.max(np.abs(g-gg))<1e-15 and np.max(np.abs(h-hh))<1e-15
    old_o,old_g,old_h=objective_gradient_hessian(x,labels,w,[.3],-.2,1.)
    assert abs((g[0]-old_g[0])-.3*(1-1/4))<1e-15
    assert abs((h[0,0]-old_h[0,0])-(1-1/4))<1e-15
    assert abs((o-old_o)-.5*.3**2*(1-1/4))<1e-15
    print(json.dumps({'synthetic_passed':True,'real_data_models_or_outcomes_read':False,
                      'tolerances_predeclared':TOL,'general_C_gradient_and_Hessian_verified':True}))


def real(revision,expected_result):
    os.chdir(ROOT);verified={};maxima={};counts=Counter();models_report=[];weights_report=[];direction_report=[]
    def verify(p,h):
    ...
    assert len(cfg['source_bindings'])==31 and len(pre['source_artifact_bindings'])==2840
    ...
    for f in FOLDS:
        ...
        assert len(fold['artifact_sha256'])==81
        for p,h in fold['artifact_sha256'].items():verify(p,h);runtime_bindings[p]=h
        data=arrays(PARENT/'fit_data'/f'fold{f}_training.npz');record=read(OUT/'provenance'/f'fold{f}_fit.json')
        old_record=read(PARENT/'provenance'/f'fold{f}_fit.json')
        assert record==fit_records[f];prov=record['fit_provenance'];bound=next(s for s in pre['support'] if s['fold']==f)
        assert record['fit_source_binding']==bound and record['new_model_fits']==4 and record['new_unique_prior_estimates']==0
        assert record['frozen_fit_data_reused'] and record['fit_labels_weights_and_priors_match_parent_exactly']
        assert prov['sample_weights']==old_record['fit_provenance']['sample_weights']
        ...
        assert old_record['fit_abs_return_mean']==absmean and old_record['fit_return_mean']==float(np.mean(returns))
        raw_logits={};raw_probability={};old_logits={};old_probability={};emitted_logits={};emitted_probability={};outer_states={}
        ...
        counts['frozen_fit_prior_verifications']+=2
        for group in GROUPS:
            ...
            for weighting in WEIGHTINGS:
                helper_id=group+'_'+weighting;mid=helper_id+'_l2unit'
                model=joblib.load(OUT/'models'/f'fold{f}_{mid}.joblib')
                old_model=joblib.load(PARENT/'models'/f'fold{f}_{helper_id}.joblib')
                assert [k for k,_ in model.steps]==['standardscaler','logisticregression']
                scaler,lr=model.steps[0][1],model.steps[1][1];state=prov['fitted_state'][helper_id]
                old_scaler,old_lr=old_model.steps[0][1],old_model.steps[1][1]
                old_state=old_record['fit_provenance']['fitted_state'][helper_id]
                outer=record['model_state'][mid];outer_states[mid]=outer
                W=float(np.sum(data['weights_'+weighting]));C=1.0/W;l2=1.0/(C*W)
                assert lr.get_params()==state['logistic_parameters']==prov['parameters']['logistic_by_model'][helper_id]
                assert lr.get_params()=={**old_lr.get_params(),'C':C}
                for k,v in {'C':C,'l1_ratio':0.,'solver':'lbfgs','tol':1e-8,'max_iter':1000,'fit_intercept':True,'random_state':20260906}.items():assert lr.get_params()[k]==v
                normalizer=state['regularization']
                assert normalizer==prov['regularization']['by_weighting'][weighting]
                for key,value in [('C',C),('solver_weight_sum',W),('actual_l2_strength',l2)]:
                    assert normalizer[key]==value and outer[key]==value
                assert normalizer['normalizer']=='C = 1.0 / float(numpy.sum(normalized_fit_sample_weight))'
                assert normalizer['rows']==nt and normalizer['weight_sha256']==matrixsha(data['weights_'+weighting])
                assert old_lr.get_params()==old_record['fit_provenance']['parameters']['logistic'] and old_lr.C==1.
                assert scaler.get_params()==old_scaler.get_params()
                for key in ('mean_','var_','scale_','n_features_in_','n_samples_seen_'):
                    exact(getattr(scaler,key),getattr(old_scaler,key),'old scaler exact '+key)
                for value in (old_lr.coef_,old_lr.intercept_,old_scaler.mean_,old_scaler.var_,old_scaler.scale_):assert np.isfinite(value).all()
                assert old_lr.coef_.tolist()==old_state['coefficient'] and old_lr.intercept_.tolist()==old_state['intercept']
                assert matrixsha(old_lr.coef_)==old_state['coefficient_sha256'] and matrixsha(old_lr.intercept_)==old_state['intercept_sha256']
                assert outer['coefficient_l2_norm']==math.sqrt(math.fsum(float(v)**2 for v in lr.coef_[0]))
                assert outer['old_coefficient_l2_norm']==math.sqrt(math.fsum(float(v)**2 for v in old_lr.coef_[0]))
                assert outer['intercept']==float(lr.intercept_[0]) and outer['old_intercept']==float(old_lr.intercept_[0])
                assert outer['unchanged_scaler_exact'] and outer['only_C_setting_changed']
                assert outer['old_model_path']==str(PARENT/'models'/f'fold{f}_{helper_id}.joblib')
                ...
                objective,gradient,hessian=objective_gradient_hessian(transformed,labels,weight,beta,intercept,C)
                ...
                assert check['solver_weight_sum']==W and check['l2_gradient_strength']==l2
                ...
                _,old_z=logit_rows(xp,old_scaler.mean_,old_scaler.scale_,old_lr.coef_[0],float(old_lr.intercept_[0]))
                old_logits[mid]=np.asarray(old_z);old_probability[mid]=np.asarray([sigmoid(v) for v in old_z])
                ...
                models_report.append({'fold':f,'model_id':mid,'fit_rows':nt,'predict_rows':len(pp),
                    'C':C,'solver_weight_sum':W,'actual_l2_strength':l2,
                    'coefficient_l2_norm':outer['coefficient_l2_norm'],'old_coefficient_l2_norm':outer['old_coefficient_l2_norm'],
                    'old_model_sha256':sha(PARENT/'models'/f'fold{f}_{helper_id}.joblib'),
                ...
                counts['models']+=1;counts['old_models_scalar_checked']+=1;counts['fit_model_rows']+=nt;counts['predict_model_rows']+=len(pp)
        for mean,mp in cfg['mapping'].items():
            ...
            for kind in ('forecasts','calibration'):
                a=arrays(OUT/kind/f'fold{f}_{mean}.npz');old_a=arrays(PARENT/kind/f"fold{f}_{mp['old_mean']}.npz")
                times=pd.DatetimeIndex(pd.to_datetime(a['timestamps'],utc=True))
                assert set(a)==set(old_a)
                for key in a:
                    if key not in ('mu','logit','probability'):exact(a[key],old_a[key],'unchanged parent field '+key)
                ...
                near(old_a['logit'][local],old_logits[mid][selected],'old_scalar_logits',maxima,TOL['logit_atol'])
                near(old_a['probability'][local],old_probability[mid][selected],'old_scalar_probabilities',maxima,TOL['probability_atol'])
                exact(np.sign(old_a['logit'][local]),np.sign(old_logits[mid][selected]),'old scalar direction signs')
                exact(np.isfinite(old_a['logit']),predicted,'old raw support')
                ...
                n=int(support.sum());assert n>0
                new_z=a['logit'][support];old_z=old_a['logit'][support]
                assert np.isfinite(new_z).all() and np.isfinite(old_z).all()
                segment='evaluation' if kind=='forecasts' else 'interval'
                diagnostic=[d for d in result['direction_diagnostics'] if (d['fold'],d['segment'],d['classifier_id'])==(f,segment,mid)]
                assert len(diagnostic)==1;diagnostic=diagnostic[0]
                expected={'fold':f,'segment':segment,'classifier_id':mid,'regime':fold['rows'][0]['regime'],'rows':n,
                    'uses_all_inference_not_score_support':True,
                    'sign_disagreements_vs_C1':int((np.sign(new_z)!=np.sign(old_z)).sum()),
                    'sign_matches_matched_prior':int((np.sign(new_z)==np.sign(record['fit_prior_logits'][mp['weighting']])).sum()),
                    'zero_logit_rows':int((new_z==0).sum()),'old_zero_logit_rows':int((old_z==0).sum()),
                    'mean_abs_logit':math.fsum(abs(float(v))/n for v in new_z),
                    'old_mean_abs_logit':math.fsum(abs(float(v))/n for v in old_z),**outer_states[mid]}
                assert expected==diagnostic
                local_diag=[d for d in fold['direction_diagnostics'] if (d['segment'],d['classifier_id'])==(segment,mid)]
                assert local_diag==[diagnostic]
                scored=a['score_support'] if kind=='forecasts' else a['interval_mask']
                assert not np.any(scored&~support)
                direction_report.append({**expected,'unscored_inference_rows':int((support&~scored).sum()),
                    'inference_timestamp_sha256':hashlib.sha256(times.asi8[support].tobytes()).hexdigest()})
                counts['direction_diagnostics']+=1;counts['old_'+kind+'_scalar_npz']+=1
        for mid in emitted_logits:
            assert np.isfinite(emitted_logits[mid]).all() and np.isfinite(emitted_probability[mid]).all()
            state=prov['fitted_state'][mid.removesuffix('_l2unit')]
    ...
    assert counts['models']==counts['old_models_scalar_checked']==32 and counts['frozen_fit_prior_verifications']==16
    assert counts['forecasts_npz']==counts['calibration_npz']==32 and len(runtime_bindings)==648
    assert counts['direction_diagnostics']==64 and len(result['direction_diagnostics'])==64
    assert counts['old_forecasts_scalar_npz']==counts['old_calibration_scalar_npz']==32
    assert {r['segment'] for r in result['scores']}=={r['segment'] for r in result['classification_scores']}=={'interval','evaluation'}
    assert result['new_model_fits']==32 and result['new_unique_priors']==0 and result['risk_model_or_calibration_fits']==0 and not result['selection_performed']
    ...
    report={'passed':True,'scope':'Independent scalar saved-model objective with actual C, stationarity, all new/old predictors, exact old scalers, surrogate support and all-inference diagnostics; no refit or policy execution',
        ...
        'models':models_report,'fit_weight_diagnostics':weights_report,'direction_diagnostics':direction_report,
        'source_bindings':cfg['source_bindings'],
        'direct_file_bindings':{str(p):sha(p) for p in (CONFIG,OUT/'registration.json',OUT/'preflight.json',OUT/'results.json')},
        'source_artifacts':2840,'runtime_artifacts':648,'distinct_hashed_files':len(verified),
