...
HF Spaces向けには、Plan004完全bundleを `C:/Users/Sophie/Documents/UniDream/unidream-space/bundles/current` に出力した。UniDream本体APIには接続しない。

- bundle type: `plan004_residual_bc_ac`
- production sample fold: 13
- source: `D_risk_sensitive_tbguard_auto_cd_floor001_pullback_evalonly`
- spec: `bc_resid_riskoff_h32`
- sample parity: `max_abs_diff=0.0`, `strict_ok=True`
- sample last position: `0.85000002`
...
Plan004検証本体は `unidream/research/plan004_residual_bc_ac.py`、本流統合ステージは `unidream/experiments/plan004_stage.py`、Space exportは `unidream/deploy/plan004_space_bundle.py` に分離した。CLIは `unidream.cli.plan004_noncompressive_bc_ac_probe` と `unidream.cli.export_plan004_space_bundle` の薄い入口だけにした。
