...
- Live / Space adoption requires a separate v31 export path and runtime wiring.
...
- It does not remove all test-selection concerns. A locked-spec untouched holdout is still needed for stronger external diligence.
...
1. Locked v31 evaluation on a fully unused holdout, ideally 2024 to present excluding the most recent live calibration window.
2. Ablation table: WM predictive state vs no-WM actor vs price-only actor vs simple momentum vs vol-target baseline.
3. Same-turnover / same-exposure random overlay baseline.
...
5. Fold-level cumulative wealth and drawdown charts.
