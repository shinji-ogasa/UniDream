...
設計:
  入力: [z_t (1024), a_t (連続 action)] × T → embed → Transformer → h_t
  出力ヘッド:
...
class TransformerWorldModel(nn.Module):
    ...
    def __init__(
        ...
        self.a_embed = nn.Linear(act_dim, d_model)
    ...
    def imagine_step(
        ...
        if past_zs is None:
            ...
            past_as = action.unsqueeze(1)  # (B, 1, act_dim)
        else:
