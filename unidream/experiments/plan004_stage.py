...
def _save_plan004_policy_npz(path: str, result: dict[str, Any], *, train_returns: np.ndarray, cfg: dict[str, Any]) -> None:
    ...
    arrays: dict[str, Any] = {
        ...
        "source": np.asarray([str(selected.get("source", ""))], dtype="<U128"),
        "spec": np.asarray([str(selected.get("spec", ""))], dtype="<U128"),
        "status": np.asarray([str(result.get("status", ""))], dtype="<U64"),
        "base_kind": np.asarray([base_kind], dtype="<U64"),
        ...
        arrays.update(
            {
                ...
                "base_mode": np.asarray([str(getattr(base_spec, "mode", ""))], dtype="<U64"),
                "base_regime": np.asarray([str(base_selection.get("regime", "all"))], dtype="<U64"),
