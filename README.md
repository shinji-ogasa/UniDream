...
Plan011 v31 は、B&H exposure `1.0` を基準にした低回転 overlay actor。現在の正しい評価定義では、VC/外部説明で守れる主成果は「B&H近傍の低回転overlayとして、collapse を避ける方向の研究証跡を作った」まで。`+3/-3` の投資判断に耐える結果ではない。

0-12 fold test-only 再集計。AlphaEx は最終値のB&H比較:
...
現行v31は、最終値AlphaExでは小幅プラス中心で、DD改善は全fold未達。
...
|---|---:|---:|
| B&H | `+0.00pt` | `+0.00pt` |
| 単純vol-target | `-1.27pt` | `-1.66pt` |
| tabular ML | `-0.38pt` | `-0.36pt` |
| Transformer WMのみ | `-1.63pt` | `-1.33pt` |
| Transformer WM + BC（ACなし） | **`+0.13pt`** | `+0.24pt` |
| Plan011 v31（WM + BC + AC） | **`+0.11pt`** | `+0.20pt` |

Transformer WM + BCは、4方式ablationで唯一AlphaEx平均をプラスにしたため、B&H-relative overlay の中核として残す価値がある。一方、AC込みv31はBC-only比でAlphaExが`-0.02pt`、MaxDDDeltaが`-0.04pt`程度の差にとどまり、AC tuningの明確な優位性は確認できていない。WM単体はDD方向のsignalを持つがalphaを失い、BC/ACを含めてもDD改善との両立は未達。詳細、fold別結果、再現hashは [policy_family_holdout_comparison.md](docs/policy_family_holdout_comparison.md) に固定している。
...
- `MaxDDDelta <= 0` は fold0-12 と holdout の両方で `0`。現状はDD改善モデルではない。
...
詳細なfold別結果・再現コマンドは [docs/plan011_v31_investor_evidence.md](docs/plan011_v31_investor_evidence.md) と [docs/plan011_v31_holdout_2024_2026.md](docs/plan011_v31_holdout_2024_2026.md) にまとめている。fold0-12 の資産推移・B&H比較・取引点チャートは [docs/figures/plan011_v31_folds0_12/README.md](docs/figures/plan011_v31_folds0_12/README.md)、holdout fold15-23 は [docs/figures/plan011_v31_holdout_folds15_23/README.md](docs/figures/plan011_v31_holdout_folds15_23/README.md) に固定している。
